...
- **Settings** (`/settings`) — incl. **Mail Workflows** (JSON-graph workflow editor — since v3.0.3 a real automation engine: AND/OR condition builder, branch/delay/webhook/child-ticket/approval/sub-workflow nodes, time-based + SLA triggers, a "Ticket updated (fields)" trigger (v3.8.21: coalesced field-change emails with the editor excluded, changed-fields/actor-kind conditions), origin-aware assign/update actions, AI proposed-reply staging with confidence-gated auto-send, installable templates) and **Ticket Ops** (SLA policies for TP-born due dates — per row a clock of 24/7, business hours or business days, the last two driven by Settings → Business Hours & Holidays; macros, custom fields, ticket links). Full node/trigger list: `AGENTS.md` → "Custom Mail Notification Workflows".
- **Visuals** (`/visuals`) — agent map/location/visibility/schedule.
- **Knowledge** (`/knowledge/*`, v3.9.89) — articles (headings = sections Auto-help reads; owner + review date), Auto-help playbooks per category/subcategory, Waiting, Activity (shadow drafts reviewed next to the team's real reply). **Shadow only**: nothing is sent to requesters. Details: `AGENTS.md` → "Knowledge + Auto-help".
...
- **Dark mode (v3.8.02, Early access) — use tokens, not `slate-*`.** Theme is class-driven (`darkMode: 'class'`; `.dark` on `<html>`, stamped by `ThemeContext` and the index.html pre-paint script). Every token above is redefined under `.dark` in `index.css`, so a surface written with tokens themes for free. Migration map: `bg-white → bg-card`, `text-slate-900/800 → text-foreground`, `text-slate-600/500/400 → text-muted-foreground`, `border-slate-200/100 → border-border`, `border-slate-300 → border-input`, `bg-slate-50/100 → bg-muted`, `hover:bg-slate-50 → hover:bg-muted`, `text-blue-600 (as a primary accent) → text-primary`. Use `dark:` twins ONLY for accent tints that have no token (e.g. `bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-200`; the convention is same hue family at ~10–20% strength). Never add a colour transition to the swap (it would fight the reduced-motion rules). `npm run lint:dark --prefix frontend` (`scripts/check-dark-tokens.mjs`) fails on NEW legacy classes anywhere under `src/` (DM-B widened it to the whole tree minus the light-only pages; `bg-white/10…30` overlay highlights and `bg-slate-800/900/950` dark chrome are exempt). For bulk work run the codemod first (`node scripts/dark-migrate.mjs <files>` — the map above + `dark:` accent twins, nested template literals handled), then hand-pass dynamic classes, inline styles and tone maps. Extra mappings the codemod uses: `text-slate-700 → text-foreground/85`, `text-slate-400 → text-muted-foreground/75`, `text-slate-300 → text-muted-foreground/50`, `bg-slate-50 → bg-muted/50`, `bg-slate-200 → bg-secondary`, `bg-slate-300/400 → bg-muted-foreground/40|60`. Charts read the tokens through `utils/highchartsTheme.js` (re-applied on the `tp:theme-changed` window event) — never hardcode chart hexes. User-supplied email HTML in threads (`.tp-rich-body`) and the Word attachment preview stay in a white content well (mail-client convention), never inverted; the composer follows the theme. Public token pages, WorkspacePicker and Login stay light (`.tp-light` on their root).
