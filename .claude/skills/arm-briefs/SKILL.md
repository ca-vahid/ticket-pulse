...
(3b) IT memo — PER-AGENT REVIEW section, from the probe's `agentReview` (ws1). This is Vahid's standup prep: the tickets to raise with each person. Three lanes: urgent_no_action (P3/P4 with zero agent activity), overdue (Open past dueBy — judge the last-note snippet: a real explanation means SKIP it, machine notes like "[Ticket Pulse] Assignment auto-assigned" do NOT count), stale_pending (Pending 5+ days without agent content). STANDUP DAYS (Tue and Thu — the team meets those mornings): comprehensive per-person lists, every kept item as a FreshService link (https://it.bgcengineering.ca/a/tickets/<fs>; TP-born tickets get https://ticketpulse.bgcsaas.com/tickets/<tp_id>), with one line each: lane, age/due, last-action date, and the ask ("update?", "park with note?", "still valid?"). Respect `truncated` markers by saying "+N more". NON-STANDUP DAYS: compress to one line per agent, only counts + the single most pressing item. ALWAYS team-safe: this is a discussion agenda, not a scoreboard — no ranking language, no cross-agent comparisons, and skip agents with nothing actionable. Curate hard: an item answered by its own last note is noise, not agenda.
