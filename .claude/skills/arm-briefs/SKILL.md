...
**VISUAL SPEC — write for a human scanning at standup, never cryptic count triads (Vahid's 23 Sep feedback: "2 · 2 · 2 (+4)" bullets are "not written for a human").** Layout, all email-safe (tables + bgcolor, no flexbox/gradients):
- Open with a dark **TEAM AT A GLANCE** strip (bgcolor #0f172a): the three lane totals as coloured labels — red "N untouched urgents", amber "N overdue without a note", grey "N long-pending".
- Then ONE CARD PER AGENT (bordered rounded cell, white bg): name left, right-aligned **coloured count chips with words** — red chip "2 untouched urgent", amber "3 overdue", slate "5 long-pending", faint "+N more". Zero-count chips are omitted.
- Inside each card, the pressing item line: small lane badge + linked ticket id (FS-born → https://it.bgcengineering.ca/a/tickets/<fs>; TP-born → https://ticketpulse.bgcsaas.com/tickets/<tp_id>) + subject + due date, ending with the ask in purple bold ("Still valid, or retire?", "Date?", "Update?"). The ask is specific to the ticket, never generic filler.
- Sort cards by attention needed (urgent desc, then overdue desc) — order is triage, not ranking. Vahid's own queue is styled differently (indigo tint, named "Your queue") and placed LAST.
- STANDUP DAYS (Tue and Thu — the team meets those mornings): each card lists EVERY kept item as its own linked line (lane badge, age/due, last-action date, the ask); respect `truncated` markers as a final "+N more" line. NON-STANDUP DAYS: one pressing item per card, as above.
- ALWAYS team-safe: a discussion agenda, not a scoreboard — no ranking language, no cross-agent comparisons; skip agents with nothing actionable; name the exclusions in a small footnote ("excluded as already-explained: …"). Curate hard: an item answered by its own last note is noise, not agenda.
