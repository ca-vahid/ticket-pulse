...
1. **Duplicates.** `CronList`. If a brief cron already exists in THIS session (prompt starts with `Daily Ticket Pulse briefs` /
   `Weekly Ticket Pulse Insights`), delete it (`CronDelete`) before creating the replacement — never two of a kind.
   Warn if another Claude session may still hold live brief crons (the July–September ones lived in the
   `ticket-pulse.cursor` session): two sessions armed = two e-mails per brief.
...
recipient override (if any) as `--to …` on the send commands.
...
### Weekly (cron `0 10 * * 5`, recurring — 10:00 AM sharp is deliberate)
```
Weekly Ticket Pulse Insights (Mon–Fri of the current week, sent Friday 10 AM PT). Recipient: Vahid only.
...
- `weekly`: `CronCreate { cron: "0 10 * * 5", recurring: true, prompt }`.
- Record the returned job ids. Compute expiry = creation + 7 days; the daily's last firing is the last weekday before
  expiry, the weekly's is the first Friday after creation (a Friday creation after 10 AM fires the following Friday).
