...
export function buildOpenApiSpec(baseUrl) {
  return {
    ...
    paths: {
      ...
      '/tickets/{id}/park': {
        post: op('Park the ticket until a date: it goes to Pending and wakes back to Open on `until`. Up to six months; a reason is required. Parking again replaces the park. Waiting on the requester is not a park (use the Pending Response status) — 400 park_requester_use_pending_response.', 'tickets:write', { tag: 'tickets', status: 201, body: { type: 'object', required: ['kind', 'until', 'reason'], properties: { kind: { type: 'string', enum: ['until_date', 'waiting_on', 'eta'], description: 'until_date = nothing to do before the date; waiting_on = waiting on a colleague / vendor / team (wakes on the chase-on date); eta = in progress with an ETA.' }, until: { type: 'string', format: 'date-time', description: 'Wake date (a bare YYYY-MM-DD wakes at 08:00 Pacific). At most six months ahead.' }, reason: { type: 'string', maxLength: 500 }, waitingOn: { type: 'array', description: 'Required for waiting_on.', items: { type: 'object', properties: { technicianId: { type: 'integer' }, email: { type: 'string' }, name: { type: 'string' } } } } } } }),
        delete: op('End the park now — the ticket reopens', 'tickets:write', { tag: 'tickets' }),
      },
      '/tickets/{id}/split': { post: op('Split a conversation out of this ticket into a new one', 'tickets:write', { tag: 'tickets', status: 201, body: { type: 'object', required: ['subject'], properties: { subject: { type: 'string', maxLength: 500 }, entryIds: { type: 'array', items: { type: 'integer' }, description: 'Thread entries to carry over. They are COPIED, never moved — an FS-sourced entry moved off its ticket would be resurrected by the next FreshService sync. Omit for a child with no history.' }, description: { type: 'string' }, requesterId: { type: 'integer', description: 'Defaults to the parent ticket’s requester.' }, priority: { type: 'integer' }, internalCategoryId: { type: 'integer' }, internalSubcategoryId: { type: 'integer' }, assignedTechId: { type: 'integer' }, moveAttachments: { type: 'boolean', description: 'Default true: attachments tied to the copied messages move to the new ticket. Ticket-level attachments always stay on the parent.' }, notifyRequester: { type: 'boolean', description: 'Default false.' } } } }) },
      '/tickets/{id}/merge': { post: op('Merge another ticket into this one', 'tickets:write', { tag: 'tickets', body: { type: 'object', description: 'Name the surviving ticket with `target` (id or display reference) or the older numeric `targetTicketId`. The ticket in the URL is closed into it.', properties: { target: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id, or any display reference the URL accepts: "TP-1504", "SR-242218", "#242218".' }, targetTicketId: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id, or any display reference the URL accepts: "TP-1504", "SR-242218", "#242218".' }, notifyRequester: { type: 'boolean' }, resolutionReason: { type: 'string', description: 'Reason stamped on the closed source. Default "duplicate" — a Security-category source demands one.' }, resolutionNote: { type: 'string' } } } }) },
      '/tickets/{id}/family': { get: op('Parent + children of a ticket', 'tickets:read', { tag: 'tickets' }) },
      '/tickets/{id}/children': { post: op('Add a child to this ticket (the parent’s side of PUT …/parent)', 'tickets:write', { tag: 'tickets', status: 201, body: { type: 'object', required: ['child'], properties: { child: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id or display reference ("TP-1504", "SR-242218", "#242218").' } } } }) },
      '/tickets/{id}/merge-many': { post: op('Merge several tickets into this one — sequential, one result per source, at most 20', 'tickets:write', { tag: 'tickets', body: { type: 'object', required: ['sources'], properties: { sources: { type: 'array', maxItems: 20, items: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id or display reference ("TP-1504", "SR-242218", "#242218").' } }, notifyRequester: { type: 'boolean' } } } }) },
      '/tickets/{id}/links': {
        get: op('Links on this ticket (related_to, duplicate_of, parent_of, merged_into) with the other ticket by reference', 'tickets:read', { tag: 'tickets', responseRef: { type: 'array', items: { type: 'object', properties: { id: { type: 'integer' }, kind: { type: 'string' }, direction: { type: 'string', enum: ['out', 'in'] }, label: { type: 'string', nullable: true }, other: { type: 'object', properties: { id: { type: 'integer' }, ref: { type: 'string' }, subject: { type: 'string' }, status: { type: 'string' } } } } } } }),
        post: op('Link this ticket to another (related_to or duplicate_of). The URL ticket is the one that changes.', 'tickets:write', { tag: 'tickets', status: 201, body: { type: 'object', required: ['related', 'kind'], properties: { related: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id or display reference ("TP-1504", "SR-242218", "#242218").' }, kind: { type: 'string', enum: ['related_to', 'duplicate_of'] } } } }),
      },
      '/tickets/{id}/links/{linkId}': { delete: op('Remove a related_to / duplicate_of link (parent_of and merged_into have their own lifecycles)', 'tickets:write', { tag: 'tickets', parameters: [{ name: 'linkId', in: 'path', required: true, schema: { type: 'integer' } }] }) },
      '/tickets/{id}/parent': {
        put: op('Set this ticket’s parent', 'tickets:write', { tag: 'tickets', body: { type: 'object', description: 'Name the parent with `parent` (id or display reference) or the older numeric `parentTicketId`.', properties: { parent: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id, or any display reference the URL accepts: "TP-1504", "SR-242218", "#242218".' }, parentTicketId: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id, or any display reference the URL accepts: "TP-1504", "SR-242218", "#242218".' } } } }),
      ...
      '/tickets/{id}/activities': {
        get: op('Audit trail: who changed what and when — status, assignee, fields, and every relation change (parent_set, parent_removed, child_added, child_removed, linked, unlinked, merged_into, merged_from, split_into, split_from, task_status_changed). Read-only', 'tickets:read', {
          tag: 'tickets',
      ...
      '/tickets/{id}/tasks': {
        ...
        post: op('Add a task (200 with `existing: true` when the externalRef already exists on this ticket, else 201)', 'tasks:write', { tag: 'tasks', body: { type: 'object', required: ['title'], properties: { title: { type: 'string' }, description: { type: 'string' }, assignedTechId: { type: 'integer', description: 'Leave out to make the task the ticket owner’s: the owner is alerted and gets the due reminder.' }, externalRef: { type: 'string', maxLength: 200, description: 'Your own key for this task. A repeat with the same externalRef on the same ticket returns the existing task (`existing: true`) instead of creating a twin.' }, dueAt: { type: 'string', format: 'date-time' }, remindBeforeMinutes: { type: 'integer', enum: [15, 30, 45, 60, 120] }, notifyAgent: { type: 'boolean' } } }, responseRef: ref('Task'), status: 201 }),
      },
        ...
        get: op('Can this person be handed a laptop or desktop? — the asset-handout gate', 'approvals:read', {
          ...
          responseRef: {
            ...
            properties: {
              ...
              approval: { type: 'object', properties: { source: { type: 'string', enum: ['ticketpulse', 'freshservice'], nullable: true, description: 'Which system holds the decisive approval — Ticket Pulse rows or the FreshService approval on the ticket (17 Sep 2026).' }, state: { type: 'string', enum: ['NOT_REQUESTED', 'PENDING', 'INFO_REQUESTED', 'EXPIRED', 'REJECTED', 'CANCELLED', 'APPROVED'] }, decidedAt: { type: 'string', format: 'date-time', nullable: true }, decidedBy: { type: 'string', nullable: true }, category: { type: 'string', nullable: true } } },
              ...
              person: { type: 'object', properties: { query: { type: 'string' }, matchedBy: { type: 'string', nullable: true, enum: ['requester_email', 'ticket_subject', 'subject_name', null] }, email: { type: 'string', nullable: true }, name: { type: 'string', nullable: true } } },
              excluded: { type: 'array', description: 'Hardware tickets this person filed AS AN IT AGENT for somebody else ("Request for Cristian Orellana : Laptop"). They never count as the agent’s own handout — listed so a surprising NO_TICKET is explainable.', items: { type: 'object' } },
