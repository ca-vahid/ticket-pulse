...
export function buildOpenApiSpec(baseUrl) {
  return {
    ...
    paths: {
      ...
      '/tickets/{id}/merge': { post: op('Merge another ticket into this one', 'tickets:write', { tag: 'tickets', body: { type: 'object', description: 'Name the surviving ticket with `target` (id or display reference) or the older numeric `targetTicketId`. The ticket in the URL is closed into it.', properties: { target: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id, or any display reference the URL accepts: "TP-1504", "SR-242218", "#242218".' }, targetTicketId: { oneOf: [{ type: 'integer' }, { type: 'string' }], description: 'A ticket id, or any display reference the URL accepts: "TP-1504", "SR-242218", "#242218".' }, notifyRequester: { type: 'boolean' } } } }) },
      ...
      '/tickets/{id}/tasks': {
        ...
        post: op('Add a task', 'tasks:write', { tag: 'tasks', body: { type: 'object', required: ['title'], properties: { title: { type: 'string' }, description: { type: 'string' }, assignedTechId: { type: 'integer', description: 'Leave out to make the task the ticket owner’s: the owner is alerted and gets the due reminder.' }, externalRef: { type: 'string', maxLength: 200, description: 'Your own key for this task. A repeat with the same externalRef on the same ticket returns the existing task (`existing: true`) instead of creating a twin.' }, dueAt: { type: 'string', format: 'date-time' }, remindBeforeMinutes: { type: 'integer', enum: [15, 30, 45, 60, 120] }, notifyAgent: { type: 'boolean' } } }, responseRef: ref('Task'), status: 201 }),
      },
