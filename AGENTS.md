...
- **Events**: Workflows trigger on `ticket.created/assigned/reassigned/resolved_closed/status_changed/fields_updated/reply_received/note_added/public_reply_added`, `approval.requested/decided/clarification_requested`, and time-based `ticket.aging`, `ticket.sla_pre_breach`, `ticket.sla_breach` (scanned by `notificationTimeTriggerService`; thresholds are trigger-node config, dispatched per-workflow via `onlyWorkflowId`). Admins can also manually run a workflow against one ticket. Resolved and closed are intentionally one event; `ticket.status_changed` is derived in `deriveTicketLifecycleEvents` for BOTH origins with `event.extra {from,to}`.
