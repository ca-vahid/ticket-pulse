...
/**
 * LEGACY per-hook native-EventSource SSE client (pre-Phase-2 behavior,
 * v3.4.00 semantics). Kept verbatim as the rollback lever: setting
 * VITE_REALTIME_TRANSPORT=eventsource makes useSSE alias this hook instead of
 * the shared realtime client — token-in-URL auth, one EventSource per
 * consumer, no transport ladder. Do not add features here; the shared client
 * (services/realtimeClient.js) is the live path.
 *
 ...
 */
export function useLegacySSE(options = {}) {
  const {
    onMessage,
    onSyncCompleted,
    onTicketChange,
