...
export const changelog = [
  ...
  },
  {
    ...
    date: 'April 23, 2026',
    entries: [
      { type: 'new', text: 'Daily Review thread hydration parallelized — the pre-LLM "Hydrating FreshService threads" loop was running one ticket at a time even though the FreshService rate limiter supports 3 concurrent in-flight requests + 110/min. Refactored to a bounded-concurrency worker pool that fans activities + conversations out as independent jobs (they hit different FS endpoints so they overlap freely). Result: a 50-ticket review now finishes in ~90 seconds instead of 30+ minutes.' },
      { type: 'new', text: 'Thread cache preheat during the regular 5-minute sync — new fire-and-forget _preheatTicketThreads tail step in performFullSync proactively pulls today\u2019s ticket cohort\u2019s activities + conversation BODIES so the daily review hits an already-warm cache and finishes in seconds instead of minutes. Cohort is keyed off FS-side state changes only (createdAt / assignedAt / resolvedAt / closedAt) — explicitly excludes Prisma\u2019s @updatedAt because it auto-bumps on every local DB write and would balloon the cohort. Per-source freshness check skips (ticket, source) pairs whose cached entry is fresher than the newest FS state change. Capped at 60 tickets per cycle so backlog drains over 1-2 cycles without blowing the FS rate-limit budget.' },
      { type: 'new', text: 'Per-workspace toggle for the daily-review preheat — new "Preheat thread cache during regular sync" toggle in Configuration → Daily Review Automation. OFF by default. Workspaces that don\u2019t use Daily Review no longer pay the FS API budget tax for conversation pulls they\u2019ll never read; admins opt-in per workspace.' },
      { type: 'new', text: 'LLM payload now carries the user\u2019s actual request — the daily-review case payload was silently dropping descriptionText AND requester (only id/name/email were even SELECTed, then stripped at payload mapping). The model was reasoning about routing decisions blind to who asked, what they asked for, where they sit, and where the assignee is located. Now ships description (truncated to 1500 chars), full requester (name/email/department/jobTitle/timeZone), and finalAssignee with location/timezone. Token cost +~1.8KB per case, well inside the 4096-token output budget.' },
      { type: 'new', text: 'New runJobsInPool utility (backend/src/utils/parallelPool.js) — small, pure bounded-concurrency worker pool used by both the daily-review hydration and the regular-sync preheat. Handles cancellation propagation, per-job error isolation, and pool sizing. Covered by 12 dedicated unit tests.' },
      { type: 'fixed', text: 'Daily Review used to fail with "The column assignment_daily_review_runs.meeting_briefing does not exist in the current database" on prod — three migrations (allow_multiple_daily_reviews_per_day, add_daily_review_meeting_briefing, add_daily_review_progress) had landed in code but never been deployed. Applied to prod via `prisma migrate deploy`.' },
      { type: 'fixed', text: 'Live ticket assignee now shown in the Decided Status column on the Assignment Review queue. After a rebound/reassignment chain (e.g. Andrew rejects → Adrian reassigned → Mehdi current) the column was showing the originally-approved tech because it preferred run.assignedTech (recorded at decision time) over the ticket\u2019s live assignedTech relation. Now trusts the ticket relation first; falls back to the run\u2019s recorded tech only when the ticket has no current owner (dismissed rows).' },
      { type: 'fixed', text: 'Removed dead `slowDelayMs: 1500` option that was being passed to the FreshService rate limiter constructor — the constructor never actually accepted it, so it was a silent no-op masquerading as a back-pressure setting.' },
      { type: 'database', text: 'New migration 20260424000000_add_daily_review_preheat_toggle — adds assignment_configs.daily_review_preheat_enabled BOOLEAN NOT NULL DEFAULT false. Applied to dev and prod. Existing rows get the safe default (off) so behavior is unchanged for workspaces until an admin opts in.' },
      { type: 'database', text: 'Three previously-unapplied migrations deployed to prod: 20260423020000_allow_multiple_daily_reviews_per_day (drops the unique index so multiple "Run Daily Review" clicks per day produce fresh rows instead of overwriting), 20260423030000_add_daily_review_meeting_briefing (adds meeting_briefing JSONB + 4 metadata columns), and 20260423040000_add_daily_review_progress (adds progress JSONB + progress_updated_at for poll-based progress UI).' },
      { type: 'security', text: 'Daily-review LLM payload still enforces the cohort allow-list — every supportingTicketIds / supportingFreshserviceTicketIds the model returns must come from analysisInput.allowedSupportingTicketIds; anything outside the set is dropped before persistence. The new richer payload (description / requester / location) does NOT widen the allow-list. Cross-workspace technician redactor (_redactForeignTechFromCase) also strips the new location + timezone fields when redacting, so the new fields can\u2019t leak across workspaces.' },
      { type: 'improved', text: 'Test coverage jumped from 75 to 97 tests across 13 suites — added parallelPool (12 tests), hydrateMissingThreads (7), preheatTicketThreads (10 incl. 2 for the new on/off gate), and dailyReviewLlmPayload (3 regression-guards covering the description + requester + tech-location contract).' },
      { type: 'improved', text: 'New diagnostic scripts under backend/scripts/ for ongoing ops: preheatDryRun.js (reports what the next preheat cycle would do without firing FS calls), reviewContextProbe.js (audits descriptionText / requester / tech.location coverage in the DB), inspectLastReview.js + diagnoseRun.js (full per-run inspection), inspectPreheatFlag.js (per-workspace flag audit).' },
    ],
  },
  {
  ...
  },
  {
    ...
    date: 'April 22, 2026',
    entries: [
      { type: 'fixed', text: 'Daily Review LLM was reading near-empty thread context for every ticket -- root cause was a silent bug in freshservice._extractResults: the generic `if (endpoint.includes(\'/tickets\'))` check matched FIRST, including for sub-resource paths like `/tickets/:id/conversations` and `/tickets/:id/activities`, so the helper returned `data.tickets` (which doesn\u2019t exist on those responses) and every paged fetch silently returned []. Compounded by the fact that the daily review only ever called `/tickets/:id/activities` (state-change events: assignments, status changes, "added a private note" *event* with no body) and never `/tickets/:id/conversations` (the only endpoint that returns reply / note BODIES). Verified before/after on local prod copy: 0 conversation rows fetched -> 82 conversation rows across 42/52 tickets, with real text like "Tier-2 verdict: Benign Positive / Authorized\u2026" reaching the LLM' },
      { type: 'new', text: 'Daily Review now fetches both /activities AND /conversations per ticket. New fetchTicketConversations(ticketId) FreshService client method walks all pages of /tickets/:id/conversations and returns the actual reply + note bodies (private + public). New transformTicketConversationEntry maps each into the existing TicketThreadEntry shape with `source: \'freshservice_conversation\'`, so both endpoint types coexist in the same table and queries stay simple. The thread excerpt builder prioritizes real conversation bodies over state-change activity entries, expanded MAX_THREAD_EXCERPTS 6 -> 12 and excerpt length 220 -> 600 chars per item, and strips HTML before passing to the model so it sees clean plaintext' },
      { type: 'new', text: 'Per-source thread coverage check before hydration -- a single Prisma groupBy(ticketId, source) query reports exactly how many activities vs conversations are cached locally for every candidate ticket, so the run only fetches what\u2019s missing instead of re-fetching everything. Force-refresh option lets the admin bypass the cache when they want fresh data from FreshService' },
      { type: 'new', text: 'Force refresh from FreshService -- new checkbox on the Daily Review trigger panel that bypasses the local thread cache and re-pulls every ticket\u2019s activities + conversations from FreshService. Slow but useful when a ticket has had recent activity since the last sync. Wired through runReview / collectDailyDataset / _hydrateMissingThreads' },
      { type: 'new', text: 'Collection Diagnostics section on the Daily Review run detail -- shows what the LLM actually had to read: candidate tickets, conversation coverage % (color-coded: green / amber / red), activity coverage %, tickets with no thread context at all, local-cache vs freshly-fetched breakdown, hydration failures, total entries by source. Surfaces the "no conversations found" case explicitly with a red callout suggesting the Force Refresh option for retry' },
      { type: 'new', text: 'Per-case payload sent to the LLM is now richer -- in addition to the joined summary string, every analyzed case also ships its full structured threadExcerpts array (each with source / eventType / visibility / actorName / occurredAt / excerpt), priority, status, episodeSummary, assignmentActions, override / decision notes, and a rebounded boolean. Same allow-list defence is unchanged: every ticket id the LLM cites must come from the cohort or it\u2019s dropped' },
      { type: 'improved', text: 'Live progress messages during the collection phase now spell out exactly what\u2019s happening: "Local cache: 38/52 fully cached. Pulling 14 missing activity stream(s) and 14 missing conversation stream(s)" instead of a generic "Hydrating thread history". Final completion message tells you how many tickets had real conversation bodies for the LLM to read, e.g. "Collection complete: 52 ticket(s) ready. 42/52 have conversation bodies for the LLM to read"' },
    ],
  },
  {
  ...
  },
  {
    ...
    date: 'April 22, 2026',
    entries: [
      { type: 'new', text: 'Daily Review -- new admin workflow that analyzes one full business day of auto-assignment outcomes for the active workspace and produces structured recommendations across three areas: prompt changes, process changes, and skill matrix changes. Each run pulls every pipeline run + every direct FreshService assignment in the workspace\u2019s configured business window, hydrates any missing FreshService thread history into a new local TicketThreadEntry table, classifies every case (success / partial success / failure / unresolved) with primary tags (rebounded / pipeline-bypassed / handled-in-FS / etc.), then sends the cohort summary + worst cases to Claude with a tool-use schema that returns concrete recommendations. Surfaced as a new Daily Review tab inside Assignment Review with run history, per-run detail, and live SSE progress (granular phases, percent, cumulative stats, activity log)' },
      { type: 'new', text: 'Approval workflow on Daily Review recommendations -- normalized assignment_daily_review_recommendations table materializes prompt/process/skill recommendations as reviewable work items with a pending \u2192 approved/rejected \u2192 applied lifecycle, reviewer/applier audit fields, optional review notes, and stable ids that survive across reruns. Each recommendation card in run detail now has Approve / Reject / Mark Applied actions; status badges + reviewer/timestamps render inline so the audit trail is visible at a glance' },
      { type: 'new', text: 'Recommendation Backlog + Weekly Approved Rollup on the Daily Review tab. Backlog filters by status (pending / approved / rejected / applied / all), kind (prompt / process / skill / all), date range, and run -- the run filter is a typeable autocomplete dropdown populated from run history (type "Run #" or any digit, blank shows everything). Weekly Rollup groups approved items by review date for the selected week and supports batch "Mark Selected Applied" so prompt-side approvals can be rolled out together' },
      { type: 'new', text: 'Persistent FreshService thread history -- new TicketThreadEntry model and ticketThreadRepository capture every comment / note / activity body locally during sync (transformTicketThreadEntries hashes each entry for idempotent upserts). The Daily Review uses this so failure analysis can quote actual conversation context instead of opaque ticket ids, and the run is no longer dependent on live FreshService API availability for already-synced tickets' },
      { type: 'new', text: 'Scheduled Daily Review -- new dailyReviewEnabled / dailyReviewRunHour / dailyReviewRunMinute / dailyReviewLookbackDays settings on AssignmentConfig let admins schedule the review to run automatically at the end of each business day. scheduledSyncService.startForWorkspace now also calls assignmentDailyReviewService.maybeRunScheduledReview which respects business-hour gates and short-circuits if the day already has a completed review unless an admin explicitly forces a rerun' },
      { type: 'new', text: 'Cancel Run -- new "Cancel Run" button on the live Daily Review view. Backend keeps an AbortController per active run; cancel propagates through every long-running step (FreshService thread hydration loop, dataset collection, the Anthropic SDK call) so an in-flight review actually stops instead of just having its DB row marked cancelled. The UI shows a dedicated cancelled state instead of falling back to "failed"' },
      { type: 'fixed', text: 'Daily Review was producing recommendations that referenced ticket numbers and technician names from outside the currently-selected workspace. Two real causes: (1) the system prompt hard-coded "You are reviewing one business day of IT auto-assignment outcomes" for every workspace, biasing the LLM into IT-flavored language; (2) the submit_daily_review_findings tool schema accepted any integer for supportingTicketIds / supportingFreshserviceTicketIds and we persisted whatever the LLM returned, so plausible-looking hallucinated ticket ids ended up stored verbatim (this is what produced ticket #219648 in the IT review even though that ticket isn\u2019t in IT). Fix: prompt now names the active workspace and explicitly says "this workspace ONLY"; the analysis input now ships explicit allowedSupportingTicketIds / allowedSupportingFreshserviceTicketIds lists; after the model returns, every supporting id is filtered against those sets before persistence and the count of dropped ids is logged + surfaced as a run warning' },
      { type: 'fixed', text: 'Cross-workspace technician detection in Daily Review -- pre-existing data integrity issues where a workspace\u2019s ticket has its assignedTechId pointing at a technician row from a different workspace are now surfaced as a run warning ("N technician reference(s) belong to other workspaces..."), and the foreign technician\u2019s name is redacted in the LLM input as "(out-of-workspace technician)" so it can\u2019t be quoted as if they were a member of the current workspace' },
      { type: 'fixed', text: 'Rebound metric in Daily Review counted rebound EVENTS, not unique tickets -- a single ticket bounced 3 times showed up as "3 rebounds". Now counts unique tickets with at least one rebound; reboundRate denominator switched from "all cases" to "unique tickets reviewed" so the percentage is honest. Same fix applied to assignmentDailyStats today/24h aggregation so the dashboard pill matches. Definition is surfaced inline on the run detail so the meaning is explicit ("Unique tickets that rebounded at least once during the review cohort. Multiple rebounds on the same ticket count once.")' },
      { type: 'fixed', text: 'Each manual rerun of the Daily Review for the same date kept showing as "Run #1" because the run table had a unique constraint on (workspaceId, reviewDate) and the service did an upsert -- so reruns overwrote the same row instead of creating a new one. Dropped the unique constraint and switched runReview() from upsert to create when force=true (manual UI clicks); each rerun now produces its own Run #N row with a fresh id. Scheduled-job calls (force=false) still short-circuit to the latest completed run for that date so the cron loop doesn\u2019t pile on duplicates. maybeRunScheduledReview switched from findUnique to findFirst ordered desc accordingly' },
      { type: 'fixed', text: 'Live Daily Review timer was stuck at "(0s)" forever -- useStreamingFetch set startTimeRef and called setElapsedSec(0) inside the same effect that wired up the SSE stream, so anything that re-triggered that effect (which the body/url object identity dance during heavy re-renders effectively did) cleared the interval and reset the start time before its first tick ever fired. Split the elapsed-time timer into its own effect that depends only on startedAt + status; the SSE setup just sets startedAt = Date.now() once per stream now' },
      { type: 'improved', text: 'Defence-in-depth workspace scoping on every Daily Review related-data query. ticketAssignmentEpisode.findMany now explicitly filters by workspaceId; ticketThreadRepository.listForTickets accepts and applies a workspaceId filter that the daily review service passes; technician selects include workspaceId so cross-workspace mismatches can be detected. The id sets the LLM is allowed to cite are now built from the actual cases sent to the model, not blindly trusted from the response' },
      { type: 'database', text: 'New migration 20260423000000_add_daily_assignment_review -- adds dailyReviewEnabled / dailyReviewRunHour / dailyReviewRunMinute / dailyReviewLookbackDays columns to assignment_configs, plus the new ticket_thread_entries table (with a unique (ticket_id, external_entry_id) index for idempotent upserts) and the assignment_daily_review_runs table' },
      { type: 'database', text: 'New migration 20260423010000_add_daily_review_recommendations -- adds the assignment_daily_review_recommendations table with the pending/approved/rejected/applied lifecycle, reviewer/applier audit fields, supporting ticket id JSON columns, and a (run_id, kind, ordinal) unique constraint so re-running a review cleanly replaces its own rows' },
      { type: 'database', text: 'New migration 20260423020000_allow_multiple_daily_reviews_per_day -- drops the old assignment_daily_review_runs unique index on (workspace_id, review_date) and replaces it with a non-unique index so each manual rerun can create its own row. Run `prisma migrate deploy` on any environment before deploying' },
    ],
  },
  {
