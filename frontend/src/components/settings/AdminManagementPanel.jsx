    ...
    <div className="space-y-6">
      ...
      {success && (
        <div className="flex items-start gap-2 p-3 bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 rounded-lg">
          ...
          <span className="text-sm text-emerald-800 dark:text-emerald-200">{success}</span>
        </div>
      )}
      ...
      {source === 'env' && (
        <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 rounded-lg">
          ...
          <div className="text-sm text-amber-800 dark:text-amber-200">
            <strong>Using environment variable.</strong> Admin emails are currently read from the <code className="bg-amber-100 dark:bg-amber-500/20 px-1 rounded">ADMIN_EMAILS</code> env var.
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-8">
          <Loader className="w-5 h-5 animate-spin text-muted-foreground/75" />
        </div>
      ) : (
            ...
            {emails.map((email) => (
              <div key={email} className="px-4 py-3 flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-muted-foreground hover:text-red-600 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-500/15 rounded-lg border border-transparent hover:border-red-200 dark:hover:border-red-500/30 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          <div className="px-4 py-3 bg-muted/50 border-t border-border">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 text-sm border border-input rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
              />
              <button
                onClick={addEmail}
