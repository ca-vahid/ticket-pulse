          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {published && (
              <button
                ...
                className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 hover:border-blue-200 hover:text-blue-700"
              >
      ...
      <section>
        ...
        {versions.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {versions.map((v) => (
              <div key={v.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2">
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  <span className="text-sm font-medium font-mono text-slate-700">v{v.version}</span>
                  <span className={`rounded border px-1.5 py-0.5 text-xs font-medium ${STATUS_BADGES[v.status] || STATUS_BADGES.archived}`}>
                    {v.status}
                  </span>
                  {v.notes && <span className="truncate text-xs text-slate-400">{v.notes}</span>}
                  <span className="text-xs text-slate-300">{formatDateOnlyInTimezone(v.createdAt, workspaceTimezone)}</span>
                </div>
                <div className="ml-2 flex items-center gap-1">
                  ...
                  {v.status !== 'published' && (
                    <button
                      ...
                      disabled={publishing}
                      className="rounded p-1.5 transition-colors hover:bg-emerald-50 disabled:opacity-50"
                      title="Publish"
                    >
