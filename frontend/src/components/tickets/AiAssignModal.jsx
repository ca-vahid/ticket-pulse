  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-4xl max-h-[88vh] flex flex-col tp-card rounded-2xl shadow-soft overflow-hidden animate-scaleIn">
        <header className="flex items-center gap-3 px-4 sm:px-5 py-3 border-b border-slate-100 bg-gradient-to-r from-indigo-50/80 via-white to-white">
          <span className="h-8 w-8 rounded-lg bg-indigo-100 text-indigo-600 inline-flex items-center justify-center flex-shrink-0">
          ...
          {hasCards && (
            <div className="hidden sm:flex items-center rounded-lg border border-slate-200 overflow-hidden text-xs font-medium">
              <button
                onClick={() => setView('cards')}
                className={`px-2.5 py-1 ${view === 'cards' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
              >
                Recommendation
              </button>
              <button
                onClick={() => setView('live')}
                className={`px-2.5 py-1 border-l border-slate-200 inline-flex items-center gap-1 ${view === 'live' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
              >
          ...
          <Link
            ...
            className="tp-focus-ring hidden md:inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:underline rounded whitespace-nowrap"
          >
        ...
        <div className="flex-1 overflow-y-auto settings-scrollbar p-4 sm:p-5">
          {loading ? (
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
              ...
              <h3 className="text-sm font-semibold text-amber-800 mb-1">Queued until the next business window</h3>
              <p className="text-sm text-amber-700">{run.queuedReason || 'Currently outside business hours.'}</p>
              <p className="text-xs text-slate-500 mt-2">It processes automatically when business hours resume — or open the full page to run it now.</p>
            </div>
          ) : hasCards && view === 'cards' ? (
            <>
              {!pending && (
              ...
              {pending && alreadyAssigned && (
                <div className="mb-3 flex items-start gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                  <Hand className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" aria-hidden="true" />
