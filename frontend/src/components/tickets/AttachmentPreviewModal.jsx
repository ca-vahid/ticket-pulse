...
export default function AttachmentPreviewModal({ ticketId, attachment, onClose }) {
  ...
  const body = () => {
      ...
      return (
        <div className="flex flex-col items-center gap-3 py-10 px-8 text-center">
          <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100">
            <FileIcon className="w-8 h-8 text-slate-400" aria-hidden="true" />
          </span>
          <div>
    ...
    if (!ready) return <Loader2 className="w-6 h-6 animate-spin text-slate-400" aria-hidden="true" />;
    if (kind === 'pdf') {
      return <iframe src={url} title={attachment.fileName} className="h-[78vh] w-[min(92vw,960px)] rounded-lg bg-white shadow-subtle" />;
    }
    ...
    if (kind === 'text') {
      return (
        <pre className="settings-scrollbar h-[72vh] w-[min(92vw,880px)] overflow-auto whitespace-pre-wrap break-words rounded-lg bg-white p-4 text-xs leading-relaxed text-slate-700 shadow-subtle">
          {rich.text}
        </pre>
      );
    }
    ...
    if (kind === 'sheet') {
      return (
        <div className="flex h-[76vh] w-[min(92vw,960px)] flex-col overflow-hidden rounded-lg bg-white shadow-subtle">
          {rich.sheetCount > 1 && (
      ...
      <div className={`relative tp-card rounded-2xl shadow-soft ${wide ? 'max-w-5xl' : 'max-w-4xl'} max-h-[92vh] flex flex-col animate-scaleIn`}>
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-slate-100">
          <span className="min-w-0 text-sm font-semibold text-slate-800 truncate">
            <FileText className="mr-1.5 inline h-4 w-4 align-[-2px] text-slate-400" aria-hidden="true" />
            {attachment.fileName}
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              title="Download"
              aria-label="Download"
              className="tp-focus-ring inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-600 hover:border-blue-300 hover:text-blue-700"
            >
