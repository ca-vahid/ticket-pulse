...
const STATUS_META = {
  open: { label: 'Open', cls: 'bg-muted text-muted-foreground border-border', Icon: Circle },
  in_progress: { label: 'In progress', cls: 'bg-amber-50 dark:bg-amber-500/15 text-amber-700 dark:text-amber-200 border-amber-200 dark:border-amber-500/30', Icon: Clock },
  done: { label: 'Done', cls: 'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-200 border-emerald-200 dark:border-emerald-500/30', Icon: Check },
};
...
const fieldLabel = 'mb-1 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground';
const fieldInput = 'tp-focus-ring w-full rounded-md border border-border bg-card px-2 py-1 text-xs text-foreground/85';
  ...
  return (
    <section className="tp-card rounded-xl p-4 sm:p-5">
      <div className="mb-3 flex items-center gap-2">
        <CheckSquare className="h-4 w-4 text-blue-600 dark:text-blue-300" aria-hidden="true" />
      ...
      {error && (
        <div className="mb-3 rounded-lg border border-red-200 dark:border-red-500/30 bg-red-50 dark:bg-red-500/15 px-3 py-2 text-xs text-red-700 dark:text-red-200" role="alert">{error}</div>
      )}

      {tasks === null ? (
        <p className="flex items-center gap-2 py-4 text-sm text-muted-foreground/75"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> Loading tasks…</p>
      ) : (
        <ul className="space-y-1.5">
          {tasks.length === 0 && <li className="rounded-lg border border-dashed border-border px-3 py-4 text-center text-sm text-muted-foreground/75">No tasks yet.</li>}
          {tasks.map((task) => {
            ...
            return (
              <li key={task.id} className="relative flex items-start gap-2.5 rounded-lg border border-border/60 px-3 py-2.5 hover:bg-muted/30">
                <button
                ...
                >
                  {busy ? <Loader2 className="h-4 w-4 animate-spin text-muted-foreground/75" aria-hidden="true" />
                    : task.status === 'done' ? <CheckSquare className="h-4 w-4 text-emerald-600 dark:text-emerald-300" aria-hidden="true" />
                      : <Circle className="h-4 w-4 text-muted-foreground/50 hover:text-emerald-500" aria-hidden="true" />}
                </button>
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-muted-foreground/75">
                    ...
                    {task.dueAt && (
                      <span
                        className={`inline-flex items-center gap-1 ${overdue ? 'font-semibold text-rose-600 dark:text-rose-300' : ''}`}
                ...
                {canWrite && (
                  <span className="flex flex-shrink-0 items-center gap-1">
                    ...
                    <select
                      ...
                      disabled={busy}
                      title="Task status"
                      className="tp-focus-ring rounded-md border border-border bg-card py-0.5 pl-1.5 pr-5 text-[11px] font-semibold text-muted-foreground"
                    >
                    ...
                    <button
                      ...
                      className="tp-focus-ring rounded p-1 text-muted-foreground/50 hover:bg-muted hover:text-muted-foreground"
                    >
                    ...
                    <select
                      ...
                      className="tp-focus-ring max-w-[120px] rounded-md border border-border bg-card py-0.5 pl-1.5 pr-5 text-[11px] text-muted-foreground"
                    >
                      <option value="">Unassigned</option>
                      {technicians.filter((t) => t.isActive !== false).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                    </select>
                    <button onClick={() => remove(task)} disabled={busy} aria-label="Delete task" className="tp-focus-ring rounded p-1 text-muted-foreground/50 hover:bg-red-50 dark:hover:bg-red-500/15 hover:text-red-500">
                  ...
                  >
                    <div className="space-y-2.5">
                      ...
                      <div>
                        ...
                        {isFsBorn && <p className="mt-1 text-[10px] leading-tight text-muted-foreground/75">FreshService sends this reminder for FS-born tasks.</p>}
                      </div>
                      <div className="flex justify-end gap-1.5 pt-0.5">
                        <button onClick={cancelEdit} className="tp-focus-ring rounded-md px-2.5 py-1 text-xs font-semibold text-muted-foreground hover:bg-muted">Cancel</button>
                        <button
      ...
      {canWrite && (
        <div className="mt-3 rounded-lg border border-border bg-muted/30 p-3">
          <div className="flex flex-wrap items-end gap-2.5">
            <div className="min-w-[180px] flex-1">
              ...
              <input
                ...
                className="tp-focus-ring w-full rounded-lg border border-border bg-card px-2.5 py-1.5 text-sm"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="tp-focus-ring rounded-lg border border-border bg-card py-1.5 pl-2 pr-6 text-sm text-muted-foreground"
              >
                <option value="">Unassigned</option>
                {technicians.filter((t) => t.isActive !== false).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
              </select>
            ...
            <div>
              ...
              <input
                ...
                className="tp-focus-ring rounded-lg border border-border bg-card px-2 py-1.5 text-sm text-muted-foreground"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="tp-focus-ring rounded-lg border border-border bg-card py-1.5 pl-2 pr-6 text-sm text-muted-foreground disabled:opacity-50"
              >
                {TIME_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
          ...
          <label className="mt-2.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
